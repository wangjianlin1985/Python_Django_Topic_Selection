...
function initTeacherManageTool() {
	teacher_manage_tool = {
		init: function() {
			//实例化编辑器
			tinyMCE.init({
				selector: "#teacher_introduce_edit",
				theme: 'advanced',
				language: "zh",
		...
		search: function() {
			...
			queryParams["professName"] = $("#professName").val();
				...
				$.ajax({
					...
					success : function (teacher, response, status) {
						...
						if (teacher) { 
							...
							$("#teacher_photoImg").attr("src", teacher.photo);
							$("#teacher_professName_edit").val(teacher.professName);
							$("#teacher_professName_edit").validatebox({
								required : true,
								missingMessage : "请输入职称",
							});
							$("#teacher_telephone_edit").val(teacher.telephone);
							$("#teacher_address_edit").val(teacher.address);
							$("#teacher_inDate_edit").datebox({
								value: teacher.inDate,
							    required: true,
							    showSeconds: true,
							});
							tinyMCE.editors['teacher_introduce_edit'].setContent(teacher.introduce);
						} else {
